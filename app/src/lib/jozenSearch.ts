 ...
 * 検索語が空の場合はnull(送信しない)。
 */
export function buildJozenFormSpec(keyword: string): JozenFormSpec | null {
...
/**
 * 画面外にformを作り、新しいタブへPOSTして即座に取り除く。
 * トップレベル遷移なのでCORSの制約は受けない。
 */
export function submitJozenSearch(keyword: string): void {
  const spec = buildJozenFormSpec(keyword);
  if (!spec) return;
